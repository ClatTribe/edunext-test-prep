...
export const Hero: React.FC = () => {
  ...
  // 1. Live User Counter Simulation
  useEffect(() => {
  ...
  // 2. Auth State Change & Profile Sync
  useEffect(() => {
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div className="flex flex-col items-start text-left">
            <div className="mb-8 animate-slide-up" style={{animationDelay: '0.1s'}}>
               <div className="inline-flex items-center space-x-2 rounded-full px-3 py-1.5" style={{ backgroundColor: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(16px)' }}>
            ...
            <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6 animate-slide-up mb-12" style={{animationDelay: '0.4s'}}>
              {/* FIXED BUTTON: Now triggers Google Login */}
              <Button 
                size="lg" 
                className="group h-14 px-8 text-lg w-full sm:w-auto" 
                onClick={handleGoogleLogin}
                disabled={loading}
              >
                {loading ? 'Connecting...' : 'Start Competing'}
                <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Button>

              <button 
                onClick={() => window.location.hash = 'analytics'} 
                className="flex items-center text-gray-300 hover:text-white transition-colors group w-full sm:w-auto justify-center sm:justify-start"
              >
                  <div className="w-10 h-10 rounded-full flex items-center justify-center mr-3 transition-all bg-white/10 group-hover:bg-[#F59E0B] group-hover:text-[#050818]">
             ...
             <div className="relative rounded-2xl overflow-hidden shadow-2xl flex flex-col" style={{ backgroundColor: secondaryBg, border: '1px solid rgba(255, 255, 255, 0.1)' }}>
                ...
                <div className="p-8 pt-6 relative z-20">
                   ...
                   <button 
                     onClick={handleGoogleLogin}
                     disabled={loading}
                     className="w-full bg-white text-gray-900 font-bold py-3.5 px-4 rounded-lg flex items-center justify-center space-x-3 transition-all shadow-lg active:scale-[0.98] hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
                   >
