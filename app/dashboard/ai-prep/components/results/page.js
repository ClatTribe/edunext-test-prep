...
const ResultAnalysis = ({ session, onBack }) => {
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      try {
        // TODO: Call Gemini API for AI analysis
        // For now, generate a simple tip
        let tip = "";
        if (accuracy >= 80) {
          tip = "Outstanding performance! You've demonstrated strong mastery across topics. Focus on maintaining consistency and tackling advanced problems.";
        } else if (accuracy >= 60) {
          tip = "Good effort! You're on the right track. Focus on your weaker topics and practice more questions to improve accuracy.";
        } else if (accuracy >= 40) {
          tip = "There's room for improvement. Review fundamental concepts in your weak areas and practice regularly to build confidence.";
        } else {
          tip = "Don't get discouraged! Start with basics, focus on understanding concepts thoroughly, and practice consistently to see improvement.";
        }
        setAiTip(tip);
      } catch (error) {
    ...
    fetchAnalysis();
  }, [score, stats.total, stats.correct, accuracy]);

  // Group by topic
  const topicBreakdown = session?.questions?.reduce((acc, q) => {
    const topic = q.topic || q.subject || 'General';
    if (!acc[topic]) acc[topic] = { total: 0, correct: 0 };
    acc[topic].total++;
    if (session?.responses?.[q.id] === q.correctAnswer) acc[topic].correct++;
    return acc;
  }, {}) || {};
    ...
    <div className="min-h-screen bg-[#020617] text-slate-100 p-8 overflow-y-auto">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-12">
           <button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-white font-black text-xs tracking-widest uppercase transition-colors">
           ...
           <section>
              ...
              <div className="glass-panel rounded-3xl p-8 border border-white/5 h-[340px] flex items-center justify-center">
                 <div className="text-center space-y-4">
                    ...
                    <p className="text-slate-500 font-medium max-w-xs">Detailed radar chart visualization of your {session?.examType?.replace('_',' ') || 'test'} strength profile will appear here.</p>
                 </div>
              </div>
           </section>
        </div>
