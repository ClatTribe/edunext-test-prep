...
import { ArrowLeft, Target, Zap, BookOpen, Brain, ChevronRight, PlayCircle, FileText, LayoutGrid, Star, Search, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function PracticeConfig({ stats, onBack, onStart, currentUserId }) {
  ...
  const fetchUserProgress = async () => {
    try {
      ...
      .from('practice_tests')
      .select('id, test_number, test_name, created_at, filters, duration_minutes')
      .order('created_at', { ascending: false });
    
    console.log('🔍 Practice Tests Fetched:', practiceTests?.length);
    console.log('🔍 First Test Filters:', practiceTests?.[0]?.filters);
    console.log('📋 Practice Tests with Filters:', practiceTests); // ⬅️ Debug log
  
      if (!practiceTests || practiceTests.length === 0) {
        console.log('❌ No practice tests found'); // ⬅️ ADD THIS
        setRecentTests([]);
        return;
      }
  
      const practiceTestIds = practiceTests.map(t => t.id);
      console.log('🎯 Practice Test IDs:', practiceTestIds); // ⬅️ ADD THIS
      ...
      console.log('📝 Attempts Found:', attempts); // ⬅️ ADD THIS
  
      if (!attempts || attempts.length === 0) {
        console.log('❌ No attempts found for this user'); // ⬅️ ADD THIS
        setRecentTests([]);
        return;
      }
      ...
      const recentTestsArray = attempts.map(attempt => {
        ...
        console.log('🎯 Test mapping:', { // ⬅️ Debug log
          test_id: test?.id,
        ...
        return {
          ...
          filters: test?.filters || null,
          duration_minutes: test?.duration_minutes || null  // ⬅️ This line must be here
        };
      });
      
      console.log('📦 Recent Tests Array with filters:', recentTestsArray); // ⬅️ Debug log
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#0E172A] text-slate-100 p-8 overflow-hidden fixed inset-0">
        ...
        <button 
  onClick={() => {
    console.log('🔙 Navigating to /dashboard/ai-prep');
    router.push('/dashboard/ai-prep');
  }}
...
{/* Recent Activity - Row 3 - ALL FILTERS */}
<div className="bg-slate-900/50 p-5 rounded-[24px] glass-border">
  <div className="flex items-center gap-2 mb-4">
  ...
  {recentTests.length === 0 ? (
    ...
    <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
      {recentTests.map((test, index) => {
        ...
        // Questions count
        filterParts.push(`${totalQuestions} Questions`);
        
        // Duration (from test config - need to fetch this)
        // For now showing if available in filters or test object
        const duration = test.duration_minutes || filters.duration_minutes;
        if (duration && duration > 0) {
          filterParts.push(`${duration} Min`);
        }
        
        // Subjects
        const subjects = filters.subjects && filters.subjects.length > 0 && !filters.subjects.includes('All') 
          ? filters.subjects.join(', ') 
          : 'All';
        filterParts.push(`Subjects: ${subjects}`);
        
        // Chapters
        if (filters.chapters && filters.chapters.length > 0 && !filters.chapters.includes('All')) {
          filterParts.push(`Chapters: ${filters.chapters.join(', ')}`);
        }
        
        // Topics
        if (filters.topics && filters.topics.length > 0 && !filters.topics.includes('All')) {
          filterParts.push(`Topics: ${filters.topics.join(', ')}`);
        }
        ...
        // Question Type
        const questionType = filters.question_type && filters.question_type !== 'mixed' 
          ? filters.question_type.toUpperCase() 
          : 'Mixed';
        if (questionType !== 'Mixed') {
          filterParts.push(`Type: ${questionType}`);
        }
        
        // Difficulty
        const difficulty = filters.difficulty && filters.difficulty !== 'mixed' 
        ...
        return (
          <div key={test.id} className="bg-white/5 p-3.5 rounded-xl yellow-border-thin hover:bg-white/8 transition-all">
            {/* Header - Test Name, Date/Time & Accuracy */}
            <div className="flex items-center justify-between mb-2.5">
              <div className="flex items-center gap-2">
                <span className="font-black text-xs" style={{ color: THEME_PRIMARY }}>
                  #{test.test_number || index + 1}
                </span>
                <span className="font-bold text-sm">{test.test_name || 'Practice Test'}</span>
                <span className="text-white font-bold text-[10px]">
                  {formattedDate} • {formattedTime}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className={`text-[9px] px-2 py-0.5 rounded-full font-black ${testAccuracy >= 70 ? 'bg-green-600' : testAccuracy >= 40 ? 'bg-amber-600' : 'bg-red-600'} text-white`}>
                  {testAccuracy}% ACC
                </span>
                <span className="text-[10px] text-slate-400 font-bold">{correctAnswers}/{totalQuestions}</span>
              </div>
            </div>
            {/* ALL Filters Line with Buttons - BOLD */}
<div className="flex items-center justify-between gap-3">
  <div className="flex items-center flex-wrap gap-x-2 gap-y-1 text-[10px] flex-1">
    {filterParts.map((part, idx) => (
      <span key={idx} className="flex items-center gap-2">
        <span className="text-slate-300 font-bold">{part}</span>
        {idx < filterParts.length - 1 && <span className="text-slate-600">•</span>}
      </span>
    ))}
  </div>
  ...
  <div className="flex items-center gap-2 flex-shrink-0">
    <button
      ...
      className="px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all hover:brightness-110 hover:scale-[1.02] active:scale-95 flex items-center gap-1.5 shadow-lg"
      style={{ backgroundColor: THEME_PRIMARY, color: '#0E172A' }}
    >
      <Brain size={13}/> Analysis
    </button>
    
    <button
      onClick={() => router.push(`/results/${test.id}/solution`)}
      className="px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all hover:brightness-110 hover:scale-[1.02] active:scale-95 flex items-center gap-1.5 bg-slate-700 text-white shadow-lg hover:bg-slate-600"
    >
      <FileText size={13}/> Solutions
    </button>
  </div>
</div>
